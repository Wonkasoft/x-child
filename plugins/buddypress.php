...
function wonkasoft_x_buddypress_enqueue_styles( $stack, $ext ) {
	wp_deregister_style( 'bp-legacy-css' );
	wp_deregister_style( 'bp-admin-bar' );
	wp_enqueue_style( 'x-buddypress', X_CHILD_TEMPLATE_URL . '/framework/dist/css/site/buddypress/' . $stack . $ext . '.css', null, X_ASSET_REV, 'all' );
}

remove_action( 'x_enqueue_styles', 'x_buddypress_enqueue_styles', 10, 2 );
add_action( 'x_enqueue_styles', 'wonkasoft_x_buddypress_enqueue_styles', 10, 2 );
...
if ( ! function_exists( 'x_buddypress_get_the_title' ) ) :
	function x_buddypress_get_the_title() {
		...
		} elseif ( bp_is_current_component( 'blogs' ) ) {
...
// Use nouveau as default template
