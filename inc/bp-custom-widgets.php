<?php
/**
 * BuddyPress Custom Members Widgets.
 *
 * @package x-child
 ...
 * @since 2.2.0
 */
function bp_custom_members_register_widgets() {
	register_widget( 'BP_Custom_Members_Widget' );
	register_widget( 'BP_Custom_Whos_Online_Widget' );
}
add_action( 'widgets_init', 'bp_custom_members_register_widgets' );
 ...
 */
function bp_custom_ajax_widget_members() {

	// check_ajax_referer( 'bp_core_widget_members' );

	// Setup some variables to check.
	$filter      = ! empty( $_POST['filter']      ) ? $_POST['filter']                : 'alphabetical';
...
add_action( 'wp_ajax_widget_members',        'bp_custom_ajax_widget_members' );
add_action( 'wp_ajax_nopriv_widget_members', 'bp_custom_ajax_widget_members' );
